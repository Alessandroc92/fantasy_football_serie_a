...
def parse_player_data(html: str) -> dict | None:
    ...
    try:
        ...
        return {
            ...
            "foot": bs.select_one(
                "span[title='Sinistro'], span[title='Destro']"
            ).get_text(),
            "nationality": bs.select_one(".nationalities").get_text(),
            "main_role": bs.select_one(".role").get("title"),
            "specific_roles": [
                role.get("title") for role in bs.select(".role.role-mantra")
            ],
            "classic_value": bs.select_one(
                "li[title='Quotazione classic'] .badge.badge-primary"
            ).get_text(),
            "mantra_value": bs.select_one(
                "li[title='Quotazione Mantra'] .badge.badge-alternative"
            ).get_text(),
            "classic_vfm": bs.select_one(
                "li[title='FantaValore di Mercato (Classic)'] .badge.badge-primary"
            ).get_text(),
            "mantra_vfm": bs.select_one(
